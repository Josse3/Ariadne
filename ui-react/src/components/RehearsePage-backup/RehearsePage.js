...
class RehearsePage extends React.Component {
    ...
    async fetchData(pageStart, pageEnd) {
        ...
        if (pageStart && pageEnd) {
            ...
            const data = await fetch(`/db/specific/${pageStart}/${pageEnd}`)
                .then(response => {
                    if (response.ok) {
                        return response.json();
                    }
                })
                .then(jsonResponse => jsonResponse.rows);
            this.setState({ dictionary: data });
        } else {
            // If not, fetch the entire JSON file
            const data = await fetch('/db/full/').then(response => response.json()).then(jsonResponse => jsonResponse.rows);
            this.setState({ dictionary: data });
        }
    }
    ...
    initializeFirstWord() {
        ...
        const remainingWords = possibleWords.slice();
    ...
    showSolution() {
        ...
        const { currentIndex } = this.state.quizdata;
        const wordPropertiesObject = this.state.quizdata.dictionary[currentIndex];
    ...
    getNewRandomWord() {
        ...
        const newRemainingWords = remainingWords.slice();
        newRemainingWords.splice(newIndex, 1);
    ...
    render() {
        ...
        const { currentIndex } = quizdata;
        const currentWordProperties = dictionary[currentIndex];
        ...
        const rehearsalHTML = (
            <div className="rehearsal">
                <h1>{currentIndex ? `${Ariadne.toGreek(dictionary[currentIndex].word)}, ${Ariadne.renderGenus(dictionary[currentIndex].genus)}` : <span className="error">Fout bij laden</span>}</h1>
